                    ...
                    <section ref="gallery" class="grid-container" :key="index">
                        ...
                        <div class="gallery-item" v-masonry="containerId" transition-duration="0.3s" item-selector=".item" :gutter="20" fit-width="true">
                            <div class="image-container item" v-for="(image, index) in sale.photos" :key="image.url_preview">
                                <div class="image-container-info-img" @click="imageClick(index)"  @contextmenu="onImageRightClick(image, $event)">
                                    <img v-masonry-tile class="img-tile" :src="image.url_preview" srcset="" :alt="image.description" />
                                </div>
                                <ContextMenu :model="items" :ref="`_menu${image.id}`"> 
                                    <template #item="{item}">
                                        <Link :href="route(item, image.id)">
